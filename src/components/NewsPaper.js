...
function NewsPaper() {
    const [newsData, setnewsData] = useState([]) // 뉴스API 데이터를 저장하는 state

    // 뉴스API 데이터를 가져오는 함수
    useEffect(() => {
        fetch('http://127.0.0.1:5300/newspaper')
            .then(res => res.json())
            .then(data => {
    ...
    // API 데이터 변수 설정
    const news = newsData[0] // 중앙기사
    const techNews = newsData[1] // 좌측기사
    const sportsNews = newsData[2] // 우측기사
