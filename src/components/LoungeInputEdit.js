...
function LoungeInputEdit({chat, index, passwordMatched, modalPosition, HandleModalEdit, confirmEditText,  dbCode, handleComment, depth, chatComment, setPasswordMatched}){
    ...
    return (
        ...
        <>
            ...
            <div className="text__function__comment"><span className="comment" onClick={(e, index)=>handleComment(e ,index)}>댓글</span> { commentNum ? <span style={{fontWeight: 'bold'}}>{commentNum}</span> : '' }</div>
        </>
        }
