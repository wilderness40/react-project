...
router.get('/', expressAsyncHandler(async (req, res) => {
    try {
        // 중앙 기사 랜덤 기사 데이터
        const response1 = await fetch(`https://newsapi.org/v2/top-headlines?pageSize=35&country=kr&apiKey=${config.Newspaper_API_KEY}`)

        const newsDatas1 = await response1.json()
        console.log(newsDatas1)
        const withImageNewsDatas1 = newsDatas1.articles.filter((newsData) => newsData.urlToImage !== null)

        // 좌측 기사 테크관련 기사 데이터
        const response2 = await fetch(`https://newsapi.org/v2/top-headlines?country=kr&category=technology&pageSize=35&apiKey=${config.Newspaper_API_KEY}`)
        ...
        // 우측 기사 스포츠관련 기사 데이터
        const response3 = await fetch(`https://newsapi.org/v2/top-headlines?country=kr&category=sports&pageSize=35&apiKey=${config.Newspaper_API_KEY}`)
